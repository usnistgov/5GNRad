...
losProbability = nrRadar.channel.computeLOSProbability(scenario, d2D, hUT);
if isBackground
    isLos = 0;
else
...
switch scenario
    ...
    case 'UMaAV'
        if hUT>= 1.5 && hUT<=22.5
            hBS1 = hBS-hUT;
            hE = nrRadar.channel.computeUTEffectiveHeight(d2D, hUT,scenario);
            hUT1 = hUT-hE;
...
sigma_SF = nrRadar.channel.computeShadowFading(scenario, isLos, hUT, isAfterBP);
