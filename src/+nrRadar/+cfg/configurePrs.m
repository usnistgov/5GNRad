function params = configurePrs(scenarioPath)
...
if ~isfile(cfgPath)
    msg = sprintf( ...
        "PRS configuration file not found.\n\n" + ...
        "scenarioPath:\n  %s\n\n" + ...
        "Expected file:\n  %s\n\n" + ...
        "Fix: ensure 'Input/prsConfig.txt' exists.", ...
        string(scenarioPath), string(cfgPath));

    throwAsCaller(MException('NIST5GNRad:MissingFile', '%s', msg));
end
...
try
    ...
    msg = sprintf( ...
        "Failed to read/parse PRS configuration file.\n\n" + ...
        "File:\n  %s\n\n" + ...
        "Cause:\n  %s\n\n" + ...
        "Expected format: tab-delimited with two columns: <paramName> <value>.", ...
        string(cfgPath), string(ME.message));

    throwAsCaller(MException('NIST5GNRad:PrsConfigParseError', '%s', msg));
end
...
params = fieldToNum(params, 'CombSize', [2 2], 'step', 1, 'defaultValue', 2);
